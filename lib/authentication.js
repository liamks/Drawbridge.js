...
Auth.prototype.makeRoutes = function(){
  ...
  /* should probably be in a dedicted class called admin */
...
Auth.prototype.checkEmail = function( req, res, next ){
  ...
  if( !util.isEmailValid( email )){
    ...
    this.db.findUserByEmail( email, function(err, result){

      if(err || result.stage !== undefined){
  ...
  if(this.options.enableScreenName){
    ...
    if( registration.screenName == ''){
      ...
      return this.db.isScreenNameBeingUsed( registration.screenName, function( err, data ){
        if( data!== null){
          errors.push('Scree name "' + registration.screenName + '" is already taken.');
        } 
        next()
      });
    }
...
Auth.prototype.users = function( req, res ){
