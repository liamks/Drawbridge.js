...
Auth.prototype.validateRegistration = function( req, res, next ){
  ...
  if(this.options.enableScreenName){
    ...
    registration.screenName = registration.screenName.toLowerCase();

    if( registration.screenName == ''  ){

      errors.push('Screen name must not be blank.');

    }else if( /[^A-Za-z0-9_\-]/.test( registration.screenName  ) ){

      errors.push('Screen names can only contain letters, numbers, or the underscore or dash characters.')
    
    }else if( this.options.restrictedScreenNames && _.include(this.options.restrictedScreenNames, registration.screenName)){

      errors.push('Sorry, you cannot use that Screen Name');
    }else{
...
Auth.prototype.processRegister = function( req, res ){
  ...
  var options = {
        message : 'A technical error occured, try again later!',
        status: 500,
  ...
  this.db.createUnactivatedUser( req.registration, (function(err, result){
    if( err ){
      ...
      emailOptions.html = this.confirmEmailTemplate({ link : link, screenName: req.registration.screenName });
      ...
      this.email.send( req.registration.email, emailOptions, function(err, obj){
        if(err){
          ...
          res.render('drawbridge/authentication/confirmationSent', {
            ...
            screenName : req.registration.screenName
          });  
        }
...
Auth.prototype.confirmRegistration = function( req, res ){
  var options = {};
  this.db.activateUser( req.params.token, (function( err, user ){
    if( err ){
      options.message = user || "Sorry, we are experiencing problems with our database, try again later";
      options.status = 500;
...
Must confirm that user has confirmed their account. If they have
not we should send them the confirmation email again.

stage == 4 // confirmed

Also recording the number of login attemps
*/
...
Auth.prototype.processLogin = function( req, res ){
  ...
  this.db.findUserByEmail( opt.email, (function( err, user ){
    ...
    if( err || user === null || user.password === undefined ){
      ...
      opt.errors = [ 'You most confirm your account via the email you were sent when you registered.'];
      res.render( 'drawbridge/authentication/login', opt );
    }else{
