...
SI.prototype.create = function(req, res){
  ...
  if( this.shouldSendConfirmationEmail && (process.env.NODE_ENV !== 'test' )){
    ...
    this.drawbridge.emit('sending-signup-email');
    this.email.send( req.info.email, info, (function(e,d){
      if(e){
        this.drawbridge.emit('sending-signup-email-error', e);
      }else{
        this.drawbridge.emit('sending-signup-email-success', d);
      }
      this.drawbridge.emit('sending-signup-email-complete');
    }).bind(this));
  }
  res.json(''); // success 
}
